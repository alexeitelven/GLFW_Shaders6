...
void main(){
	initSpheres();
	seed = iTime + iResolution.y * gl_FragCoord.x / iResolution.x + gl_FragCoord.y / iResolution.y;
...
#else
		colora += radiance(Ray(camPos, normalize(.53135 * (iResolution.x/iResolution.y*uv.x * cx + uv.y * cy) + cz)));
#endif
    }
	color = vec4(pow(clamp(colora/float(SAMPLES), 0., 1.), vec3(1./2.2)), 1.);
}
